...
Nocturne Manga 是一个**纯本地运行**的漫画浏览系统，无需外部网络服务。整体架构分为两层：
...
class ServerContext:
    ...
    state: dict             # {"lock": Lock, "allowed_pdf_paths": set, "allowed_output_paths": set}
...
`state` 字典中的路径集合在 `handle_refresh()` 后会更新，确保新添加的 PDF 立即可访问。
...
Jinja2 生成的 HTML 包含:
    ...
    ├── <script src="catalog.js" defer>
    └── <script src="context_menu.js" defer>
    │
    ▼
catalog.js IIFE 执行:
    ├── initTheme()        # 读取 localStorage，应用暗色/亮色主题
    ├── initSidebar()      # 恢复侧边栏状态（展开/收起/宽度）
    ├── buildTree()        # 从 CONFIG.tree 构建 DOM 目录树
    ├── bindEvents()       # 绑定搜索、排序、快捷键、卡片点击等事件
    └── 暴露 window.CatalogApp API
```
...
renderPdf(pdf, card)
    ...
    │     │  对于每一页 (1..N):
    │     │     ├── pdf.getPage(n)
    │     │     ├── page.getViewport({scale: pixelRatio})
    │     │     ├── page.render({canvasContext, viewport})
    │     │     ├── canvas.toBlob("image/jpeg", 0.92)
    │     │     └── URL.createObjectURL(blob) → 加入图片列表
    │     │
...
2. 环境变量 (COMICREAD_RANGE_SUPPORT, COMICREAD_NO_BROWSER_OPEN)
3. 代码默认值 (lib/config.py 常量, builder.py catalog_config)
```
...
│
├── catalog.css                # ↓ 以下由 copy_runtime_assets() 复制
├── catalog.js                 #   仅在 size 或 mtime 变化时复制
├── context_menu.js
├── css/
│   ├── base.css
│   ├── cards.css
