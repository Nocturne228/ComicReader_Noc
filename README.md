...
```bash
echo "alias comic='cd ~/Documents/manga/ComicReadScript && conda run --no-capture-output -n ai python -u catalog.py ~/Documents/manga/pdf --serve'" >> ~/.zshrc
```
...
comic ~/Documents/manga --serve --host 0.0.0.0
```

## 命令行参考

```
usage: catalog.py [-h] [--serve] [--host HOST] [--port PORT] [--output-dir OUTPUT_DIR] folder
...
| 一键全部展开/折叠 | 工具栏「全部展开 / 全部折叠」按钮同时控制侧边栏树和主界面分组 |
| 搜索过滤 | 侧边栏搜索框输入关键词，实时过滤目录树 |
| 排序切换 | 顶部下拉菜单：按名称 / 按修改时间 |
| 打开方式切换 | macOS 的 `--serve` 模式下使用「网页阅读 / Preview 打开」按钮切换卡片点击行为 |
| 侧边栏控制 | `◀` 收起侧边栏，`☰` 展开；状态自动记忆 |
| 手动刷新目录 | `--serve` 模式下点击「刷新目录」按钮，重新扫描 PDF 文件夹并刷新页面 |
| 关闭本地服务 | `--serve` 模式下点击「关闭服务」按钮，从网页端停止 HTTP 服务 |
...
├── vendor/
│   └── pdfjs/                 # pdf.js 与 worker（自动复制，离线可用）
└── images/
    ├── a-<hash>.jpg           # PDF 首页封面（JPEG, 150 DPI）
    ├── b-<hash>.jpg           # 文件名含相对路径 hash，避免同名冲突
    └── ...
```
...
| `image` | string | 对应封面 JPEG 文件名（标题 + 相对路径 hash） |
