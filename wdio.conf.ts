...
export const config: WebdriverIO.Config = {
  ...
  before: async () => {
    ...
    const uuid = await $(
      'li[data-qa-target-type="extension"]:has([title="暴力猴"]) .fieldpair:nth-child(2) dd:nth-child(2)',
    ).getText();

    // 添加脚本
    await browser.url(
      `moz-extension://${uuid}/options/index.html#scripts/_new`,
    );
